Attribute VB_Name = "ModuleGenerateRDN"
' Génère un relevé de notes pour un étudiant en format PDF ou XLSX
...
Function GenerateRDN(ByVal Matricule As String, _
                      Optional ByVal Format As String = "PDF", _
                      Optional ByVal filename As String = "ReleveDeNotes_[DATE]_[NAME]_[SURNAME]", _
...
Private Sub ReadData(ByVal Matricule As String, ByRef Person As Object, _
    ...
    Dim countDateNull As Integer
    Dim successfulCredits As String
        ...
        If WsPointGrill.Cells(1, i).value <> "" Then
            ...
            If UBound(courseParts) = 1 Then
                ...
                If InStr(1, UCase(Trim(WsPointGrill.Cells(2, i).value)), "UE", vbTextCompare) > 0 Then
                    ...
                    successfulCredits = WsPointGrill.Cells(position, i + 1).value
                Else
                ...
                values(5) = Trim(WsPointGrill.Cells(position, i).value)
                values(6) = Trim(WsPointGrill.Cells(3, i).value)
                
                If successfulCredits = "N" Then
                    values(7) = 0
                ElseIf successfulCredits = "O" Then
                    values(7) = values(6)
                Else
                    values(7) = "-"
                End If
                ...
                Else
                    ...
                    ReDim Preserve Table(1 To 7, 1 To n)
    ...
    For numberBloc = 1 To 3
        ...
        ReDim reverseTable(1 To UBound(Table, 2), 1 To 7)
    ...
    If Info = "Minimum" Then
        For numberBloc = 1 To 3
            ...
            For i = 1 To UBound(Table, 1)
                If Table(i, 5) = "-" Then
                    countDateNull = countDateNull + 1
                End If
            Next i
            
            If countDateNull >= UBound(Table, 1) Then
                Data.Remove "Bloc_" & numberBloc
            End If
        Next numberBloc
    End If
End Sub
...
Private Sub CreateSheet(ByVal ws As Worksheet, Person As Object, Data As Object)
    ...
    For index = 0 To Data.count - 1
        ...
        With ws.Range(Cells(10, 2 + colOffset), Cells(11, 3 + colOffset))
            .Merge
            .value = "Code ECTS"
            .Font.Italic = True
        End With
        
        With ws.Range(Cells(10, 4 + colOffset), Cells(11, 5 + colOffset))
            .Merge
            .value = "Nom d'UE / AcAp"
            .Font.Italic = True
        End With
        
        With ws.Range(Cells(10, 6 + colOffset), Cells(11, 6 + colOffset))
            .Merge
            .value = "Qua"
            .Font.Italic = True
        End With
        
        With ws.Range(Cells(10, 7 + colOffset), Cells(11, 7 + colOffset))
            .Merge
            .value = "Notes /10"
            .WrapText = True
            .Font.Italic = True
        ...
        For i = 1 To DataRow
            For j = 1 To 7
                Select Case j
                    ...
                    Case 2
                        ws.Range(Cells(12 + i - 1, 2 + colOffset), Cells(12 + i - 1, 3 + colOffset)).Select
                        Selection.Merge
        
                    Case 3
                        ws.Range(Cells(12 + i - 1, 4 + colOffset), Cells(12 + i - 1, 5 + colOffset)).Select
                        Selection.Merge
        
                    Case 4
                        ws.Cells(12 + i - 1, 6 + colOffset).Select
        
                    Case 5
                        ws.Cells(12 + i - 1, 7 + colOffset).Select
                ...
                With Selection
                    ...
                    If (j = 5) Then
                        .value = Replace(Data(Data.Keys()(index))(i, j), ",", ".")
                    Else
        ...
        ws.Range(Cells(12 + DataRow, 2 + colOffset), Cells(12 + DataRow, 3 + colOffset)).Merge
        ws.Range(Cells(12 + DataRow, 4 + colOffset), Cells(12 + DataRow, 5 + colOffset)).Merge
        
        With ws.Cells(12 + DataRow, 6 + colOffset)
            .value = "Total"
        ...
        With ws.Cells(12 + DataRow, 7 + colOffset)
            criteriaRange = ws.Range(Cells(12, 1 + colOffset), Cells(12 + DataRow - 1, 9 + colOffset)).Address
            sumRange = ws.Range(Cells(12, 7 + colOffset), Cells(12 + DataRow - 1, 7 + colOffset)).Address
        ...
        With ws.Cells(12 + DataRow, 8 + colOffset)
            criteriaRange = ws.Range(Cells(12, 1 + colOffset), Cells(12 + DataRow - 1, 9 + colOffset)).Address
            sumRange = ws.Range(Cells(12, 8 + colOffset), Cells(12 + DataRow - 1, 8 + colOffset)).Address
